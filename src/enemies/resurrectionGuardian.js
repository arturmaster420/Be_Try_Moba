...
export function createResurrectionGuardian(zone, pos) {
  ...
  enemy.onHitPlayer = (self, player, state) => {
    if (!player._lvlUpInvuln && !player._lvlUpChoosing) {
      player.hp -= self.damage * (typeof self._barrierDebuffUntil === "number" && state && state.time < self._barrierDebuffUntil ? (self._barrierDmgMult || 1) : 1);
    }
    // Targeting 2.0 memory
    player.lastAttacker = self;
    player.lastAttackerAt = (state && state.time) || 0;
  };
