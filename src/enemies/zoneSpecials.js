...
import { pickMobTarget } from "./utils.js";
...
export function createZone4Spawner(zone, pos) {
  ...
  enemy.update = (self, dt, state) => {
    ...
    const vx = (dx / dist) * self.speed * 0.6;
    const vy = (dy / dist) * self.speed * 0.6;
    ...
    // Spawn basic mobs periodically (up to a soft cap around)
    self._spawnTimer += dt;

    if (self._spawnTimer >= self._spawnInterval) {
      self._spawnTimer = 0;

      // Soft cap: if already many enemies nearby, skip
      let nearby = 0;
      const maxNearby = 10;
      for (const e of enemies) {
        if (e === self) continue;
        if (e.type !== "basic") continue;
        const ddx = e.x - self.x;
        const ddy = e.y - self.y;
        if (ddx * ddx + ddy * ddy <= 500 * 500) {
          nearby++;
      ...
      if (nearby < maxNearby) {
        ...
        for (let i = 0; i < count; i++) {
          ...
          enemies.push(createBasicMob(zone, { x: sx, y: sy }));
        }
      }
    }
  };
...
export function createZone5Champion(zone, pos) {
  ...
  enemy.update = (self, dt, state) => {
    ...
    const vx = (dx / dist) * self.speed * 0.9;
    const vy = (dy / dist) * self.speed * 0.9;
