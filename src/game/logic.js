...
function pushNotification(state, text) {
  ...
  state.notifications.push({ text, t: 2 });
}
...
function levelUp(state) {
  ...
  p.magnetRadius = Math.min(p.magnetRadius, MAX_MAGNET);
  p.hp = p.maxHp;
  state.xpToNext = Math.floor((10 + state.level * 3) * 0.5);
  let msg = null;
  if (state.level === 10) msg = "Rifle unlocked (LVL 10)";
  else if (state.level === 20) msg = "Shotgun unlocked (LVL 20)";
  else if (state.level === 30) msg = "Rocket Launcher unlocked (LVL 30)";
  else if (state.level === 40) msg = "LASER unlocked (LVL 40)";
  if (msg) pushNotification(state, msg);
}
...
export function updateGame(state, input, dt) {
  ...
  if (state.notifications && state.notifications.length) {
    ...
    state.notifications = state.notifications.filter((n) => n.t > 0);
  }
  ...
  if (weapon.type === "laser") {
    ...
    if (shooting && state.fireCooldown <= 0) {
      ...
      const range =
        ...
        p.rangeMul *
        (state.tempBuffs.tempRange > 0 ? 1.5 : 1);
      const speed = Math.min(weapon.bulletSpeed * p.bulletSpeedMul, 2000);
      ...
      for (let i = 0; i < pellets; i++) {
        ...
        state.bullets.push({
          x: p.x + ax * (p.radius + 8),
          y: p.y + ay * (p.radius + 8),
          prevX: p.x + ax * (p.radius + 8),
          prevY: p.y + ay * (p.radius + 8),
          vx: ax * speed,
          vy: ay * speed,
          r: 4,
          life: range / speed,
  ...
  for (const e of state.enemies) {
    ...
    if (dist > 1) {
      const nx = dx / dist;
      const ny = dy / dist;
      let speed = def.speed ? def.speed(state.wave) : 60;

      // ограничиваем скорость всех врагов до 95% текущей скорости игрока
      const playerSpeed = state.player.speed || state.player.baseSpeed || 220;
      const maxEnemySpeed = playerSpeed * 0.95;
      if (speed > maxEnemySpeed) speed = maxEnemySpeed;
  ...
  for (let i = state.bullets.length - 1; i >= 0; i--) {
    ...
    let hitX = b.x;
    let hitY = b.y;
    const sx = b.x - (b.prevX ?? b.x);
    const sy = b.y - (b.prevY ?? b.y);
    const segLen2 = sx * sx + sy * sy;
    for (const e of state.enemies) {
      let px = b.x;
      let py = b.y;
      if (segLen2 > 0) {
        const t = ((e.x - b.prevX) * sx + (e.y - b.prevY) * sy) / segLen2;
