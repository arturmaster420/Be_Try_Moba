...
const MAX_CRIT_MULT = 19998; // в 2 раза выше, по сути без лимита
...
function applyPermanentPickup(state, type) {
  ...
  switch (type) {
    ...
    case "hpBig":
      ...
      break;
    case "radius":
      p.magnetRadius *= 1.005; // +0.5%
      p.magnetRadius = Math.min(p.magnetRadius, MAX_MAGNET);
      break;
    case "fireRate":
      p.fireRateMul *= 1 + 0.003; // 0.6%
    ...
    case "damage":
      ...
      break;
    case "critChance":
      p.critChance = clamp(p.critChance + 0.005, 0, 0.5);
      break;
    case "critDamage":
      p.critMult *= 1 + 0.003;
...
function levelUp(state) {
  state.level += 1;
  const p = state.player;
  // +2.5% к базовым статам (кроме критов и HP)
  p.damageMul *= 1.025;
  p.fireRateMul *= 1.025;
  p.rangeMul *= 1.025;
  p.bulletSpeedMul *= 1.025;
  p.magnetRadius *= 1.025;
  p.magnetRadius = Math.min(p.magnetRadius, MAX_MAGNET);
  p.hp = p.maxHp;
...
export function updateGame(state, input, dt) {
  ...
  if (weapon.type === "laser") {
    ...
    const trueRate = Math.min(baseRate * rateMul, 3); // кэп 3/сек
    const cd = 1 / trueRate;
  ...
  for (const e of state.enemies) {
    ...
    if (dist > 1) {
      ...
      // враги никогда не быстрее игрока (<= 95% базовой скорости)
      {
        const basePlayerSpeed =
          state.player.baseSpeed || state.player.speed || 220;
        const maxEnemySpeed = basePlayerSpeed * 0.95;
