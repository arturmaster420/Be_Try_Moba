...
export function renderGame(ctx, state, viewW, viewH) {
  ...
  const cam = state.cam;
  const offsetX = viewW / 2 - cam.x;
  const offsetY = viewH / 2 - cam.y * 0.9; // камера чуть поднята
  ...
  for (const e of state.enemies) {
    ...
    ctx.fillRect(s.x - barW / 2, s.y - e.r - 10, barW, barH);
    ctx.fillStyle = "#22c55e";
    ctx.fillRect(s.x - barW / 2, s.y - e.r - 10, barW * hpRatio, barH);
  }
  ...
  ctx.beginPath();
  ctx.arc(ps.x, ps.y, state.player.radius, 0, Math.PI * 2);
