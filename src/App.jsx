...
moveRef.current = mv;
  ...
  const handleSwitchWeapon = () => {
    ...
    // порядок прогрессии
    const order = ["pistol", "rifle", "shotgun", "rocket", "laser"];

    // какие оружия сейчас доступны по уровню
    const available = [];
    if (lvl >= 1) available.push("pistol");
    if (lvl >= 10) available.push("rifle");
    if (lvl >= 20) available.push("shotgun");
    if (lvl >= 30) available.push("rocket");
    if (lvl >= 40) available.push("laser");

    if (!available.length) return;

    const currentKey = st.selectedWeaponKey || available[available.length - 1];
    let idx = available.indexOf(currentKey);
    if (idx === -1) {
      idx = available.length - 1;
    }
    const nextKey = available[(idx + 1) % available.length];
