...
function satellitesParams(player) {
  ...
  // - Satellite count increases at lvl 5 and lvl 9 (max 3 in-run)
  // - Shop meta-level can add +2 more satellites (extra at lvl 5 and lvl 9)
  ...
  let count = 1;
  if (lvl >= 5) count += 1 + extraAt5;
  if (lvl >= 9) count += 1 + extraAt9;
  count = Math.min(5, Math.max(1, count));

  // Orbit radius (acts like range) – grows steadily.
  const orbitR = (56 + (lvl - 1) * 3.0 + Math.max(0, count - 1) * 2.0) * (0.85 + (rMult - 1) * 0.35);
...
function spiritParams(player) {
  ...
  // metaLvl 2 = +1 extra spirit at lvl 5
  // metaLvl 3+ = +1 extra spirit at lvl 9
...
function summonParams(player) {
  ...
  return { level: lvl, count, hp, def, moveSpeed, tauntR, radius, cooldown, lureR, tauntHold };
}
