...
function satellitesParams(player) {
  ...
  // Magic Survival inspired progression:
  // - Early levels mostly improve damage/speed
  // - Satellite count increases steadily as you level it
  // - Size and tick-rate improve in small steps
  const COUNT_BY_LVL = [0, 1, 1, 2, 2, 3, 3, 4, 4, 5, 6];
  const SIZE_BONUS =   [0, 0, 0, 1, 1, 1, 2, 2, 3, 3, 4];

  const count = Math.min(8, COUNT_BY_LVL[lvl] || 1);
  const orbR = 9 + (SIZE_BONUS[lvl] || 0);

  // Orbit radius grows gently (range affects it, but less than linear)
  const orbitR = (54 + (lvl - 1) * 2.2 + Math.max(0, count - 1) * 1.5) * (0.85 + (rMult - 1) * 0.35);

  // Per-hit damage is modest because count scales up.
  const hitDamage = (10 + (lvl - 1) * 2.2) * dMult;

  // Tick improves a bit over levels; keep a floor.
  const tick = Math.max(0.12, 0.22 - (lvl - 1) * 0.007);

  // Rotation speed ramps slightly.
  const orbitSpeed = 1.15 + (lvl - 1) * 0.045;
...
function energyBarrierParams(player) {
  ...
  return { level: lvl, radius, pushSpeed, pulseDamage, tick, slowMult, dmgMult };
}
