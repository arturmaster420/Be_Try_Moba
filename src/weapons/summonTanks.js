...
// - They do NOT attack.
// - In-run (lvl 1..10): +1 tank at lvl 5, +1 at lvl 9 (max 3).
// - Shop meta upgrades can add +2 more (extra at lvl 5 and lvl 9).
...
export function updateSummonTanks(player, state, dt, params) {
  ...
  for (let i = 0; i < owned.length; i++) {
    ...
    // Never let tanks drift too close to the hero; push them out a bit.
    const px = s.x - player.x;
    const py = s.y - player.y;
    const pd = Math.hypot(px, py) || 1;
    const minD = followDist * 0.85;
    if (pd < minD) {
      const push = (minD - pd);
      s.x += (px / pd) * push;
      s.y += (py / pd) * push;
