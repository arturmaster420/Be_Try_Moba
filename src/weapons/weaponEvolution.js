...
export function updateWeapon(player, state, dt, aimDir, isFiring) {
  ...
  if (stage === 3) {
    updateLaser(player, state, dt, aimDir, isFiring);
    return;
  }

  if (!isFiring || !aimDir) return;
  ...
  if (stage === 1) {
    const params = bulletParamsForLevel(player.level);
    fireBullets(player, state, dir, params);
  } else if (stage === 2) {
    const params = rocketParamsForLevel(player.level);
    fireRockets(player, state, dir, params);
  } else if (stage === 4) {
    const params = lightningParamsForLevel(player.level);
    fireChainLightning(player, state, params, dir);
  }
