...
export function ensureShopMeta(prog) {
  ...
  // Satellites are unlocked by default (available in run-upgrade pool),
  // but are not granted at run start.
  if ((prog.skillMeta && (prog.skillMeta["skill:satellites"] | 0) <= 0)) {
    prog.skillMeta["skill:satellites"] = 1;
  }
