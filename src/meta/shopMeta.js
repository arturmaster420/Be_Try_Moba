...
// Defaults: keep current core gameplay intact.
// - bullets + bombs available by default
// - advanced attacks (laser/lightning) locked until purchased
// - all passives available by default (can be upgraded in shop later)
...
export function ensureShopMeta(prog) {
  ...
  if (!prog.shopOffers || typeof prog.shopOffers !== "object") prog.shopOffers = { active: [], passive: [] };
...
export function ensureShopOffers(prog) {
  ...
  while (offers.active.length < 3) {
    const it = pickRandom(CATALOG.active, used);
    if (!it) break;
...
export function replaceOfferSlot(prog, kind, idx) {
  ...
  const list = kind === "passive" ? offers.passive : offers.active;
  const pool = kind === "passive" ? CATALOG.passive : CATALOG.active;

  const used = new Set([...offers.active, ...offers.passive]);
