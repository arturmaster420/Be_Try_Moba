  ...
  ensureCellsAroundPlayer(player) {
    ...
    for (let dy = -radius; dy <= radius; dy++) {
      for (let dx = -radius; dx <= radius; dx++) {
        ...
        // Ambient orbs: a few XP/coin pickups that simply lie on the map (not only from mobs).
        // Spawn once per generated cell (streaming) so exploration feels rewarding.
        this.spawnAmbientOrbsForCell(zone, center);
      }
    }
  }

  spawnAmbientOrbsForCell(zone, center) {
    const state = this.state;
    if (!state || !Array.isArray(state.xpOrbs)) return;
    const z0 = zone | 0;
    if (z0 === 0) return;

    // Higher chance in early zones so the player clearly sees them.
    const chance = z0 <= 2 ? 0.65 : z0 <= 4 ? 0.45 : 0.35;
    if (Math.random() > chance) return;

    // Ambient orb density multiplier (player request: 3x more orbs lying on the map).
    const ORB_MULT = 3;

    // Keep density modest (cells are generated in a 7x7 / 9x9 area).
    const baseCount = z0 <= 2 ? randInt(1, 2) : randInt(0, 2);
