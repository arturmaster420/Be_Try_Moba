...
export class SpawnSystem {
  constructor(state) {
    ...
    this.cellSize = 2400; // increased density
  ...
  createGroup(zone, type, center) {
    ...
    const g = {
      ...
      activationDist: type === "camp" ? 5200 : type === "patrol" ? 4800 : 4500,
    };
  ...
  spawnGroup(g) {
    ...
    if (g.type === "herd") {
      const count = Math.floor(randRange(5, 11));
    ...
    if (g.type === "camp") {
      ...
      const minions = Math.floor(randRange(4, 9));
  ...
  ensureCellsAroundPlayer() {
    ...
    const radius = 3; // explore more cells around player
    for (let dy = -radius; dy <= radius; dy++) {
      for (let dx = -radius; dx <= radius; dx++) {
        ...
        const extra = (zone >= 4 && Math.random() < 0.35) ? 1 : 0;
