import { getZone, ZONE_RADII, ZONE6_SQUARE_HALF, ZONE6_CORNER_POINTS, isPointInHub } from "./zoneController.js";
...
export class SpawnSystem {
  ...
  spawnAmbientPickupsInCell(zone, cx, cy) {
    const state = this.state;
    if (!state || !Array.isArray(state.xpOrbs)) return;
    const z = zone | 0;
    if (z === 0) return; // Hub: keep clean

    // Keep ambient pickups lightweight. They are generated once per streamed cell.
    ...
    // Coin chance slightly increases with zone, but stays modest to avoid farm spam.
    const coinChance = Math.min(0.32, 0.14 + z * 0.03);

    // Spawn within cell bounds (small margin).
    const margin = 140;
    const minX = cx * cs + margin;
    const maxX = (cx + 1) * cs - margin;
    const minY = cy * cs + margin;
    const maxY = (cy + 1) * cs - margin;
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (isCoin) {
        ...
        state.xpOrbs.push({
          ...
          coins,
          age: 0,
          // Ambient pickups should not accumulate forever across streamed cells.
          ttl: randRange(70, 110),
          source: "ambient",
        });
      } else {
        const baseXp =
  ...
  ensureCellsAroundPlayer(player) {
    ...
    for (let dy = -radius; dy <= radius; dy++) {
      for (let dx = -radius; dx <= radius; dx++) {
        ...
        // Ambient pickups: XP/Coin orbs that already lie on the map.
        this.spawnAmbientPickupsInCell(zone, cx, cy);
