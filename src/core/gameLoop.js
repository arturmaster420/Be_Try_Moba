...
export function createGame(canvas, ctx, progression) {
  ...
  startNewRun(state);  function update(dt) {
    state.time += dt;
    ...
    // Global pause: stop world simulation but keep HUD/UI running
    if (state.paused) {
      updatePopups(state, dt);
    ...
    // Permanent HP regeneration from progression limits (HP Regen Limit)
    const limits = state.progression?.limits || {};
    const regenLimit = limits.hpRegen || 0;
    if (regenLimit > 0 && state.player.hp > 0) {
      const regenPerSec = regenLimit;
      state._hpRegenAccumulator += regenPerSec * dt;
      if (state._hpRegenAccumulator >= 1) {
        const whole = Math.floor(state._hpRegenAccumulator);
        state._hpRegenAccumulator -= whole;
        state.player.hp = Math.min(
          state.player.hp + whole,
          state.player.maxHP
        );
      }
    }
...
function render() {
    ...
    // Floating texts in screen-space with subtle shadow for readability
  ...
  function handlePointerDown(x, y) {
    ...
    // Pause button hit-test (always available in playing mode)
    const rect = state._pauseButtonRect;
    if (rect) {
      if (
        x >= rect.x &&
        x <= rect.x + rect.w &&
...
function renderFloatingTexts(ctx, state) {
  const { floatingTexts, canvas } = state;
  ...
  for (const t of floatingTexts) {
    const alpha = Math.max(0, Math.min(1, t.time / 1.2));
