...
export function createGame(canvas, ctx, progression) {
  ...
  function update(dt) {
    ...
    // Permanent HP regen limit (HP/s) from progression
    const regenLimit = state.progression?.limits?.hpRegen || 0;
    if (regenLimit > 0 && state.player.hp > 0) {
      state._hpRegenAccumulator += regenLimit * dt;
      if (state._hpRegenAccumulator >= 1) {
        const whole = Math.floor(state._hpRegenAccumulator);
        state._hpRegenAccumulator -= whole;
        state.player.hp = Math.min(state.player.maxHP, state.player.hp + whole);
      }
    }
