...
// - rings are already tied to zones via ring borders below
// - we only draw 12 subtle rays here (no HUD text yet)
{
	...
	// Keep very subtle; background should not overpower gameplay.
	const baseAlpha = isSmallMobile ? 0.05 : 0.07;
	ctx.save();
	...
	for (let i = 0; i < 12; i++) {
		...
		ctx.beginPath();
		ctx.moveTo(0, 0);
		ctx.lineTo(dx * maxLen, dy * maxLen);
		ctx.stroke();
	}

	// Emphasize cardinal axes a bit (12/3/6/9)
	ctx.globalAlpha = Math.min(1, baseAlpha * 1.7);
