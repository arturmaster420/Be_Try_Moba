...
export function createGame(canvas, ctx, progression) {
  ...
  const state = {
    ...
    // Host-authoritative run-upgrade session (pauses all players while one chooses)
    _runUpNet: {
      active: false,
      targetId: null,
      choices: null,
...
// Initialize a preview run so the world/player can render behind the menu.
  ...
  state.net.onMessage = (msg) => {
    ...
    if (msg.type === "runPause") {
      if (state.net.isHost) return;
      state.paused = true;
      state._netRunPaused = true;
      return;
    }

    if (msg.type === "runResume") {
      if (state.net.isHost) return;
      state._netRunPaused = false;
    ...
    if (msg.type === "runChoices") {
      ...
      showRunUpgradeOverlay(
        ...
        (choice) => {
          ...
          state._runUpgradeActive = false;
          // Stay paused until host resumes.
        }
      );
      return;
    }
    ...
    if (msg.type === "runPick") {
      ...
      hostHandleRunPick(state, from, choiceId);
      return;
    }
  };
  ...
  function updateSim(dt) {
    ...
    if (online && state.net && state.net.isHost) {
      if (hostMaybeStartRunUpgradeSession(state)) {
        // Host paused the world and initiated a choice (local UI or sent to a joiner).
        return;
      }
    } else {
      // Offline singleplayer
      if (maybeOpenRunUpgradeOverlay(state)) {
        return;
      }
    }
...
function updateWeapons(state, dt, online) {
  ...
  if (state.player) {
    const blockedByOverlay = !!(online && state.overlayMode);
    const blockedByRunUpgrade = !!state._runUpgradeActive;
...
function maybeOpenRunUpgradeOverlay(state) {
  ...
  showRunUpgradeOverlay(
    ...
    (choice) => {
      ...
      // If multiple level-ups were earned at once, keep choosing until done.
      if ((p._pendingLevelUps || 0) > 0) {
        // Defer to next tick to avoid nested focus issues.
        setTimeout(() => {
          if (state.mode === "playing") {
            maybeOpenRunUpgradeOverlay(state);
          }
        }, 0);
      } else {
        state.paused = false;
...
// Host-authoritative run-upgrade flow:
// While any player has pending level-ups, pause the whole world and let exactly one player choose.
// Host applies the choice, then either prompts the same player again (if multiple pending) or the next pending player.
function hostMaybeStartRunUpgradeSession(state) {
  if (!state || state.mode !== "playing") return false;
  if (!isOnline(state) || !state.net?.isHost) return false;
  if (state.overlayMode) return false;
  if (state._runUpNet?.active) return true;
  if (state._runUpgradeActive) return true;
  if (state.paused) return false;

  const ps = getPlayersArr(state);
  const target = ps.find((p) => p && p.hp > 0 && (p._pendingLevelUps || 0) > 0);
  if (!target) return false;

  hostStartRunUpgradeForPlayer(state, target);
  return true;
}

function hostStartRunUpgradeForPlayer(state, player) {
  if (!state || !player) return;
  const tid = String(player.id);
  const localId = state.player ? String(state.player.id) : "";

  const choices = rollRunUpgrades(player, 3);
  state._runUpNet = state._runUpNet || { active: false, targetId: null, choices: null };
  state._runUpNet.active = true;
  state._runUpNet.targetId = tid;
  state._runUpNet.choices = choices;

  // Pause host sim + tell everyone to pause.
  state.paused = true;
  state._runUpgradeActive = true;
  if (state.net && state.net.status === "connected") {
    state.net.sendRunPause("levelUp", tid);
  }

  const pending = player._pendingLevelUps || 0;
  const metaText = `${player.nickname || "Player"} · Lv ${player.level} · Pending ${pending}`;

  if (tid === localId) {
    showRunUpgradeOverlay(choices, metaText, (choice) => {
      hostHandleRunPick(state, tid, choice?.id || "");
    });
  } else {
    // Send choices to the target joiner. Everyone receives, but only 'to' will show the overlay.
    if (state.net && state.net.status === "connected") {
      state.net.sendRunChoices(tid, choices, metaText);
    }
  }
}

function hostHandleRunPick(state, fromId, choiceId) {
  if (!state || !state.net?.isHost) return;
  const sid = String(fromId || "");
  if (!sid) return;
  const session = state._runUpNet;
  if (!session || !session.active) return;
  if (String(session.targetId) !== sid) return;
  ...
  if (!player) {
    ...
    if (state.net && state.net.status === "connected") state.net.sendRunResume();
    return;
  }
  ...
  // Continue with the same player if they have more pending.
  if ((player._pendingLevelUps || 0) > 0) {
    // Re-roll and re-prompt
    session.choices = rollRunUpgrades(player, 3);
    const tid = sid;
    const localId = state.player ? String(state.player.id) : "";
    const metaText = `${player.nickname || "Player"} · Lv ${player.level} · Pending ${player._pendingLevelUps}`;

    if (tid === localId) {
      setTimeout(() => {
        showRunUpgradeOverlay(session.choices, metaText, (choice) => {
          hostHandleRunPick(state, tid, choice?.id || "");
        });
      }, 0);
    } else {
      if (state.net && state.net.status === "connected") {
        state.net.sendRunChoices(tid, session.choices, metaText);
      }
    }
    return;
  }
  ...
  const next = getPlayersArr(state).find((p) => p && p.hp > 0 && (p._pendingLevelUps || 0) > 0);
  if (next) {
    // Switch target (keep paused)
    setTimeout(() => {
      hostStartRunUpgradeForPlayer(state, next);
    }, 0);
    return;
  }

  // Done: resume everyone.
  session.active = false;
...
function serializePlayerState(state) {
  ...
  const encSkills = (p) => {
    const s = p?.runSkills || {};
    // bullets,bombs,rockets,laser,lightning
    return [s.bullets|0, s.bombs|0, s.rockets|0, s.laser|0, s.lightning|0].join(",");
  };
...
function applyPlayerStateToClient(state, pstate) {
  ...
  for (const sp of (pstate.players || [])) {
    ...
    // Sync run skill levels from host (for HUD + aim range consistency on joiners).
