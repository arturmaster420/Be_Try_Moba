...
export function createGame(canvas, ctx, progression) {
  ...
  function update(dt) {
    ...
    var limits = (state.progression && state.progression.limits) || {};
    var regenLimit = limits.hpRegen || 0;
    if (regenLimit > 0 && state.player.hp > 0) {
      var regenPerSec = regenLimit; // HP per second
      state._hpRegenAccumulator += regenPerSec * dt;
      if (state._hpRegenAccumulator >= 1) {
        var whole = Math.floor(state._hpRegenAccumulator);
...
function render() {
    ...
    // Floating texts in screen-space with subtle shadow
...
function startNewRun(state) {
  ...
  state.paused = false;
  state._hpRegenAccumulator = 0;
  state._pauseButtonRect = null;
