...
// Initialize a preview run so the world/player can render behind the menu.
  ...
  function updateSim(dt) {
    ...
    if (online && state.net && state.net.isHost) {
      ...
      // Offline singleplayer (classic pause)
...
function hostProcessRunUpgrades(state) {
  ...
  for (const p of getPlayersArr(state)) {
    ...
    // Local host player: show overlay only if not already active and no other overlay is blocking.
    if (id === localId) {
      if (state._runUpgradeActive) {
      ...
      if (state.overlayMode) continue; // don't pop run-upgrades on top of death/upgrade screens
      if (!s) {
        const choices = rollRunUpgrades(p, 3);
        ...
        showRunUpgradeOverlay(choices, metaText, (choice) => {
          hostApplyRunPick(state, id, choice?.id || "");
        });
      }
      continue;
    }

    // Remote player: if no session yet, send choices once.
    if (!s) {
      const choices = rollRunUpgrades(p, 3);
      const metaText = `${p.nickname || "Player"} · Lv ${p.level} · Pending ${pending}`;
      sessions.set(id, { choices, metaText, sentAt: nowT });
...
function updateXPOrbs(state, dt) {
  ...
  for (let i = xpOrbs.length - 1; i >= 0; i--) {
    ...
    // Keep orb world position stable; bob is visual-only.
    orb._bob = Math.sin(orb.age * 5) * 2;

    // Ambient orbs have ttl to avoid accumulating across streamed world cells.
...
function renderXPOrbs(state, ctx) {
  ...
  for (const orb of state.xpOrbs) {
    const oy = orb.y + (orb._bob || 0);
    const kind = orb.kind || (orb.coins ? "coin" : "xp");
    ctx.fillStyle = (kind === "coin") ? "#ffd34a" : "#5af2ff";
    ctx.beginPath();
    ctx.arc(orb.x, oy, orb.radius || 8, 0, Math.PI * 2);
    ctx.fill();
    if (kind === "coin") {
      // small inner highlight
      ctx.fillStyle = "rgba(255,255,255,0.55)";
      ctx.beginPath();
      ctx.arc(orb.x - 2, oy - 2, Math.max(1, (orb.radius || 8) * 0.25), 0, Math.PI * 2);
      ctx.fill();
    }
  }
  ctx.restore();
