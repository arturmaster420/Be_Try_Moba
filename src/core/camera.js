export class Camera {
  constructor(canvas) {
    ...
    // Cached info about canvas / aspect
    this._lastW = 0;
    this._lastH = 0;
    this._aspect = 1;
  ...
  update(player, dt) {
    ...
    if (this._isMobile) {
      // Mobile:
      // Portrait  → контролируем вертикаль (Y)
      // Landscape → контролируем горизонталь (X)
      const axisPixels = this._isLandscape ? w : h;
      targetZoom = axisPixels / (paddedRange * 2);
    } else {
      // Desktop / large canvas: сохраняем старую идею с aspect
