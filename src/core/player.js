...
export class Player {
  constructor(startPos, startLevel) {
    ...
    this.runSkills = { bullets: 1, bombs: 0, rockets: 0, laser: 0, lightning: 0 };
    this.runEvolutions = {};
    this.runPassives = {
      damage: 0,
      attackSpeed: 0,
  ...
  xpToNext() {
    // Default leveling speed (no x2 boost)
    return Math.max(1, Math.floor(20 + this.level * 5));
  }
  ...
  render(ctx) {
    ...
    const auraR = emojiR; // ring comes straight from emoji, no extra radius
	    ...
	    // Aura behind the emoji (thin ring, no glow)
    ctx.save();
    ctx.globalCompositeOperation = "destination-over";

	    // Thin main ring (no shadow, no outer glow)
    ctx.beginPath();
