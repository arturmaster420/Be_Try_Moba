...
export const META_TIERS = [
  ...
  },
  {
    ...
    maxPoints: {
      ...
      score: 100,
      pickupRadius: 100,
      // Crits only become available on Tier 5
...
export function getMaxPointsForStat(metaTier, statKey) {
  const tier = META_TIERS.find((t) => t.id === metaTier) || META_TIERS[0];
  if (!tier.maxPoints) return Infinity;
  const max = tier.maxPoints[statKey];
...
export function loadProgression() {
  try {
    ...
    // v3: introduce metaTier + soft caps by Tier
    if (data.metaVersion < 3) {
      if (typeof data.metaTier !== "number") {
        data.metaTier = 1;
      }
      data.metaVersion = 3;
    }
    ...
    // Clamp any overcapped stats for this Tier
    for (const key of Object.keys(limits)) {
