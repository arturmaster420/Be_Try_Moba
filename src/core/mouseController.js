...
export function getAimDirectionForPlayer(player, camera, canvas) {
  // Portrait mobile: aim = movement direction (one-finger control).
  if (isPortraitMobile()) {
    const dx = state.moveVecX;
    const dy = state.moveVecY;
    const len = Math.hypot(dx, dy);
    if (len > 0.1) {
      return { x: dx / len, y: dy / len };
    }
    // If no movement input, no fresh aim direction; let caller handle null.
    return null;
  }
