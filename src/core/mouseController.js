...
export function isFiringActive() {
  if (isPortraitMobile()) {
    // Portrait auto-fire: shooting is gated by aimDir & cooldown,
    // not by holding the finger. Always allow firing logic to run.
    return true;
  }
  // Desktop / landscape mobile: fire only while mouse or aim-stick is active.
  ...
  if (isPortraitMobile()) {
    ...
    if (best) {
      ...
      return { x: best.dx / len, y: best.dy / len };
        ...
        // No valid target in range — no aim in portrait mode (no shots).
    return null;
  }
