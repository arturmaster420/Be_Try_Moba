...
const attackKeys = ["bullets", "bombs", "satellites", "energyBarrier", "spirit", "electricZone", "laser", "lightning", "rockets"];
...
for (const s of RUN_SKILLS) {
  ...
  const skipAntiSnowball = (s.key === "energyBarrier" || s.key === "spirit" || s.key === "electricZone");
...
export function describeRunUpgrade(player, up) {
  ...
  if (up.kind === "skill") {
    ...
    if (up.key === "spirit") {
      return up.from <= 0 ? "Summon a spirit (shoots enemies)" : `Lv ${up.from} → ${up.to}: +damage / faster`;
    }
