...
export function describeRunUpgrade(player, up) {
  ...
  if (up.kind === "skill") {
    ...
    if (up.key === "spirit") {
      return up.from <= 0 ? "Spirit flames (auto basic shots)" : `Lv ${up.from} → ${up.to}: +range / +atkspd / +dmg (extra spirits at 5 & 9)`;
    }
    if (up.key === "summon") {
      return up.from <= 0 ? "Summon tanks (taunt + soak)" : `Lv ${up.from} → ${up.to}: +HP / +DEF / faster respawn (extra tanks at 5 & 9)`;
    }
