...
export const RUN_SKILLS = [
  { key: "bullets", name: "Basic Shot", kind: "skill" },
...
export function applyRunDerivedStats(player) {
  ...
  // Multipliers are intentionally modest. No caps in-run.
  player.runDamageMult = 1 + (p.damage || 0) * 0.10;
  player.runAttackMult = 1 + (p.attackSpeed || 0) * 0.06;
  player.runMoveMult = 1 + (p.moveSpeed || 0) * 0.05;
  player.runRangeMult = 1 + (p.range || 0) * 0.06;

  player.runPickupBonusRadius = (p.pickupRadius || 0) * 10;
  player.runXpGainMult = 1 + (p.xpGain || 0) * 0.06;
...
const MAX_SKILL_LEVEL = { bullets: 10, bombs: 10, rockets: 10, laser: 10, lightning: 10 };
const evo = player.runEvolutions || {};

const attackKeys = ["bullets", "bombs", "laser", "lightning", "rockets"];
...
const canFuseRockets =
  !evo.rocketFusion &&
  (skills.bullets | 0) >= (MAX_SKILL_LEVEL.bullets || 10) &&
  (skills.bombs | 0) >= (MAX_SKILL_LEVEL.bombs || 10) &&
  (skills.rockets | 0) <= 0;
...
for (const s of RUN_SKILLS) {
  ...
  const hasAnyExtraSkill = (skills.bombs || 0) > 0 || (skills.laser || 0) > 0 || (skills.lightning || 0) > 0 || (skills.rockets || 0) > 0;
...
export function describeRunUpgrade(player, up) {
  ...
  if (up.kind === "evolution") {
    if (up.key === "rocketFusion") {
      return "Fuse Basic Shot (MAX) + Bombs (MAX) → Rockets (Lv1). Consumes both.";
    }
    return "Evolution";
  }
  ...
  if (up.kind === "skill") {
    if (up.key === "bullets") {
      return `Lv ${up.from} → ${up.to}: +shot power / sometimes +extra bullets`;
    }
    if (up.key === "bombs") {
