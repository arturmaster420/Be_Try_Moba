...
export function renderUpgradeMenu(ctx, state) {
  ...
  items.forEach((item, idx) => {
    ...
    const btnW = 56 * s * mobileBoost;
    const btnH = 28 * s * mobileBoost;
  ...
  const mobileBoost = canvas.width < 768 ? 1.2 : 1.0;
  const startBtnW = 220 * s * mobileBoost;
  const startBtnH = 48 * s * mobileBoost;
  const startBtn = {
    x: w / 2 - startBtnW / 2,
    y: h / 2 + 120 * s,
    w: startBtnW,
