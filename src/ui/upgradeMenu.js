...
export function renderUpgradeMenu(ctx, state) {
  ...
  ctx.fillText("YOU DIED", w / 2, headerY0);
  ...
  yHeader += headerFontSize * 1.5;
  ctx.fillText(
    "R-Tier: " + rTier + " (Used Points: " + usedPoints + " / " + totalMax + ")",
    w / 2,
    yHeader
  );
  ...
  if (isMobile && !isLandscape) {
    ...
    for (const item of leftItems) {
      ...
      if (Number.isFinite(maxForStat)) {
        ctx.textAlign = "right";
        ctx.fillStyle = "#ffffff";
        const capText = `${val}/${maxForStat} очки`;
  ...
  // START button — всегда внутри экрана
  const btnH = 40;
  let startBtnY = lastItemY + listFontSize * 2.0;
