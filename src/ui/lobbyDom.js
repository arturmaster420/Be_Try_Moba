...
function connectAndDo(state, action) {
  ...
  _pendingNetAction = { seq, action, nickname, avatarIndex, roomCode };

  const runAction = (req) => {
    if (!req) return;
    try {
      if (req.action === "host") net.host(req.roomCode, req.nickname, req.avatarIndex);
      else if (req.action === "join") net.join(req.roomCode, req.nickname, req.avatarIndex);
      else net.fastJoin(req.nickname, req.avatarIndex);
    } catch {}
  };
...
function renderShop(state) {
  ...
  const offers = prog.shopOffers || { active: [], passive: [] };
