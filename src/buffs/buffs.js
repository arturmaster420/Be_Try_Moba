...
export function updateBuffs(state, dt) {
  ...
  const baseDamage =
    (player._weaponDamage || player.baseDamage) * metaDamageMult;
  const baseAttackSpeed =
    (player._weaponAttackSpeed || player.baseAttackSpeed) * metaAttackMult;

  player.damage = baseDamage * (1 + damageBoost);
  player.attackSpeed = baseAttackSpeed * (1 + attackSpeedBoost);
  player.moveSpeed = player.baseMoveSpeed * metaMoveMult * (1 + moveSpeedBoost);

  // expose temporary XP gain boost for XP orb collection
